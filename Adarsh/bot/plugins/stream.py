...
async def login_handler(c: Client, m: Message):
    try:
        try:
            ag = await m.reply_text("Now send me password.\n\n for password send 10rs to `skvillageboys@okhdfcbank` and send screen shot to @kwicadmin \n\n (for running bot need contributions..unwanted message = ban + report)")
...
async def private_receive_handler(c: Client, m: Message):
    if MY_PASS:
        ...
        if check_pass== None:
            await m.reply_text("ʟᴏɢɪɴ ғɪʀsᴛ ᴜsɪɴɢ /login ᴄᴍᴅ(ᴛʏᴘᴇ /login)  \n\n for password send 10rs to `skvillageboys@okhdfcbank` and send screen shot to @SK_MoviesOffl \n\n (for running bot need contributions..unwanted message = ban + report)")
            return
